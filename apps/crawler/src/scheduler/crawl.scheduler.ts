...
import { RunScheduledCrawlUseCase, type CrawlConfigSnapshot } from "@relaymap/crawling";
import { CRAWLER_CONFIG, type CrawlerConfig } from "../config/crawler-config.js";
...
export class CrawlScheduler implements OnModuleInit, OnModuleDestroy {
	...
	onModuleInit(): void {
		...
		job.start();
		this.logger.log(`Scheduled crawl registered with cron "${this.config.cronSchedule}"`);
	}
	...
	async handleScheduledCrawl(): Promise<void> {
		...
		this.running = true;
		try {
			await this.runCrawl.execute(this.toCrawlConfig(), this.abortController.signal);
		} catch (err) {
			this.logger.error(`Crawl failed: ${(err as Error).message}`, (err as Error).stack);
		} finally {
			this.running = false;
		}
	}
